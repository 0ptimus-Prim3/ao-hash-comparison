...
#include "hash/mimc/mimc512f2k.hpp"

template<typename FieldT>
class mimc512f2k_two_to_one_hash_gadget : public GadgetPP<FieldT>
{
public:
    using super = GadgetPP<FieldT>;
    using Hash = Mimc512F2K<FieldT>;
...
public:
    ...
    void generate_r1cs_witness()
    {
        ...
        // Optimize first round
        // h0 = x1^3
        val(inter[i]) = val(x[1]) * val(x[1]);
        ++i;
        val(inter[i]) = val(inter[i - 1]) * val(x[1]);
        ++i;
        ...
        {
            // h0 <- (h0+x0+c)^3+h1
            t = val(inter[i - 1]) + val(x[j & 1]) + round_c[j];
            val(inter[i]) = t * t;
            ++i;
            // (h1 = inter[i-4])
            val(inter[i]) = val(inter[i - 1]) * t;
        ...
        for (size_t j = 0; j < ROUNDS_N - 3; ++j)
        {
            t = val(inter[i - 1]) + val(y[j & 1]) + round_c[j];
            val(inter[i]) = t * t;
            ++i;
            val(inter[i]) = val(inter[i - 1]) * t;
            val(inter[i]) += val(inter[i - 4]);
        ...
        // explicit last two rounds, as we put the results in out[0] and out[1]
        // second to last round
        t = val(inter[i - 1]) + val(y[(ROUNDS_N - 3) & 1]) + round_c[ROUNDS_N - 3];
        val(inter[i]) = t * t;
        ++i;
        ...
        // last round
        t = val(out[1]) + val(y[(ROUNDS_N - 2) & 1]) + round_c[ROUNDS_N - 2];
        val(inter[i]) = t * t;
        ++i;
